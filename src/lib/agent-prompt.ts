...
export function buildAgentPrompt(config: ResolvedAppConfig, metadata: SiteMetadata): string {
  const lines = [
    ...
    `2. Run: npx -y appbun@latest ${shellQuote(config.url)} --name ${shellQuote(config.name)} --out-dir ${shellQuote(config.outDir)} --titlebar ${config.titlebar} --browser-engine ${config.browserEngine} --width ${config.width} --height ${config.height} --theme-color ${shellQuote(config.themeColor)} --yes`,
